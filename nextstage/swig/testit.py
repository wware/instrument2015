...
adsr.keydown(1)
adsr.step()
assert adsr.output() == 0xf3a562, hex(adsr.output())
adsr.step()
assert adsr.output() == 0x1e74ac5, hex(adsr.output())
adsr.step()
assert adsr.output() == 0x2daf027, hex(adsr.output())
adsr.step()
assert adsr.output() == 0x3ce958a, hex(adsr.output())
...
for i in range(4000):
    adsr.step()
assert adsr.output() == 0x5f5688d1, hex(adsr.output())
for i in range(4000):
    adsr.step()
assert adsr.output() == 0x2312a88c, hex(adsr.output())
adsr.keydown(0)
for i in range(5):
    adsr.step()
assert adsr.output() == 0x2307808c, hex(adsr.output())
for i in range(25000):
    adsr.step()
assert adsr.output() == 0x114fb2, hex(adsr.output())
